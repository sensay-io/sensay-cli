...
export async function reportCommand(
  ...
  try {
    ...
    if (!apiKey) {
      console.error(
        chalk.red(
          '❌ No API key found. Please run "sensay claim-key" first or provide --apikey option.'
        )
      );
      process.exit(1);
    }
    ...
    const mainSpinner = progress.createSpinner(
      "report",
    ...
    try {
      ...
      const knowledgeBasePromises = replicas.map(async (replica) => {
        try {
          const response =
            await KnowledgeBaseService.getV1ReplicasKnowledgeBase(
              replica.uuid!
            );
  ...
  if (replicas.length === 0) {
    ...
    replicas.forEach((replica) => {
      ...
      const systemMessage = replica.systemMessage
        ...
        : "N/A";
      const created = replica.createdAt
        ? new Date(replica.createdAt).toLocaleDateString()
        : "N/A";

      lines.push(
        `| ${name} | ${model} | ${uuid} | ${systemMessage} | ${created} |`
      );
    });
  }
  ...
  replicas.forEach((replica) => {
    ...
    if (items.length > 0) {
      ...
      items.forEach((item) => {
        ...
        const type = item.file.mimeType || "N/A";
        const size = item.file.size
          ? (item.file.size >>> 10) + "." + (item.file.size & 0x3ff) + "KB"
          : "N/A";
...
export function setupReportCommand(program: Command): void {
  ...
  const cmd = program
    ...
    .option(
      "-o, --output <path>",
      "Output file path (default: ./organization-report.md)"
    )
