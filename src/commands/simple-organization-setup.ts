...
import { ApiError, UsersService, ReplicasService } from "../generated/index";
...
async function scanForReplicaFolders(
  targetPath: string
): Promise<ReplicaFolder[]> {
...
export async function simpleOrganizationSetupCommand(
  folderPath?: string,
  ...
  try {
    ...
    if (replicaFolders.length === 0) {
      console.error(
        chalk.red(
          "❌ No replica folders found. Each replica folder should contain a system-message.txt file."
        )
      );
      process.exit(1);
    }
    ...
    console.log(
      chalk.blue(
        `📦 Found ${replicaFolders.length} replica${
          replicaFolders.length > 1 ? "s" : ""
        } to process:\n`
      )
    );
    replicaFolders.forEach((folder) => {
      console.log(
        chalk.cyan(`  - ${folder.name} (model: ${folder.modelName})`)
      );
    });
    console.log();
    ...
    if (!effectiveConfig.apiKey) {
      console.error(
        chalk.red('❌ No API key found. Please run "sensay claim-key" first.')
      );
    ...
    if (!userName || !userEmail) {
      ...
      if (options.nonInteractive) {
        ...
        if (!currentConfig.userName || !currentConfig.userEmail) {
          console.error(
            chalk.red(
              "❌ Missing required configuration. In non-interactive mode, you must either:"
            )
          );
          console.error(
            chalk.red(
              "   1. Provide command line options: --user-name, --user-email"
            )
          );
          console.error(
            chalk.red(
              "   2. Or have these values in your project config file (sensay.config.json)"
            )
          );
          process.exit(1);
        }
        ...
        const questions = [
          {
            ...
            validate: (input: string) =>
              input.trim().length > 0 || "User name is required",
          },
          {
            type: "input",
            name: "userEmail",
    ...
    const userSpinner = progress.createSpinner(
      "user",
    ...
    const existingReplicasSpinner = progress.createSpinner(
      "fetch-replicas",
      "Fetching existing replicas..."
    );
    ...
    try {
      ...
      existingReplicasSpinner.succeed(
        `Found ${existingReplicas.length} existing replicas`
      );
    } catch (error) {
    ...
    for (let i = 0; i < replicaFolders.length; i++) {
      ...
      console.log(
        chalk.blue(
          `\n🤖 Processing replica ${i + 1}/${replicaFolders.length}: ${
            replicaFolder.name
          }\n`
        )
      );
      ...
      try {
        ...
        const replicaSpinner = progress.createSpinner(
          `replica-${i}`,
          `Checking for existing replica: ${replicaFolder.name}...`
        );

        let replica;
        const existingReplica = existingReplicas.find(
          (r) => r.name === replicaFolder.name
        );
        ...
        if (existingReplica) {
          ...
          await ReplicasService.putV1Replicas(
            existingReplica.uuid,
            "2025-03-25",
            {
              name: replicaFolder.name,
              shortDescription:
                existingReplica.shortDescription ||
                `AI replica for ${replicaFolder.name}`,
              greeting:
                existingReplica.greeting || "Hello! How can I help you today?",
              ownerID: user.id,
              slug: existingReplica.slug, // Keep the existing slug
              llm: {
                model:
                  (replicaFolder.modelName as any) || "claude-3-5-haiku-latest",
                memoryMode: "rag-search",
                systemMessage:
                  replicaFolder.systemMessage ||
          ...
          replicaSpinner.succeed(
            `Updated existing replica: ${replica.name} (model: ${replicaFolder.modelName})`
          );
        } else {
          ...
          const slug =
            user.id.slice(0, 5) +
            replicaFolder.name.toLowerCase().replace(/[^a-z0-9]/g, "-");

          const replicaCreateResponse = await ReplicasService.postV1Replicas(
            "2025-03-25",
            {
              name: replicaFolder.name,
          ...
          if (replicaCreateResponse.success && replicaCreateResponse.uuid) {
            replica = await ReplicasService.getV1Replicas1(
              replicaCreateResponse.uuid
            );
            replicaSpinner.succeed(
              `Created new replica: ${replica.name} (model: ${replicaFolder.modelName})`
            );
          } else {
        ...
        const { files, skipped } = await FileProcessor.scanTrainingFiles(
          replicaFolder.path
        );
        ...
        try {
          await FileProcessor.clearExistingTrainingData(
            replica.uuid,
            options.force,
          ...
          if (
            error.message.includes("Use --force to automatically delete it")
          ) {
            ...
            console.log(
              chalk.yellow(
                "⚠️  Warning: Could not clear existing training data completely"
              )
            );
        ...
        if (files.length === 0) {
          console.log(
            chalk.yellow("⚠️  No training data found in training-data folder")
          );
          console.log(
            chalk.blue(
              "ℹ️  Replica training data has been cleared and is ready for new content"
            )
          );
        } else {
          ...
          const trainingSpinner = progress.createSpinner(
            `training-${i}`,
            "Uploading training data..."
          );

          try {
            uploadResults = await FileProcessor.uploadTrainingFiles(
              replica.uuid,
              files,
              trainingSpinner
            );
            ...
            if (failed > 0) {
              trainingSpinner.succeed(
                `Training data upload completed: ${successful} successful, ${failed} failed`
              );
              ...
              uploadResults
                ...
                .forEach((r) => {
                  console.log(
                    chalk.gray(`   - ${r.file.relativePath}: ${r.error}`)
                  );
                });
            } else {
              trainingSpinner.succeed(
                `Training data uploaded: ${files.length} files processed`
              );
            }
          } catch (error: any) {
            trainingSpinner.fail(
              `Training data upload failed: ${error.message}`
            );
            // Don't throw - just warn and continue
            console.log(
              chalk.yellow(
                "⚠️  Some training files may not have uploaded successfully"
              )
            );
          }
    ...
    if (processedReplicas.length > 0) {
      console.log(
        chalk.green(
          `\n✅ Successfully processed ${processedReplicas.length} replica${
            processedReplicas.length > 1 ? "s" : ""
          }:`
        )
      );
      processedReplicas.forEach((r) => {
        ...
        console.log(
          chalk.gray(
            `    Model: ${r.model}, Training files: ${r.trainingFiles}`
          )
        );
      });
    }
    ...
    if (failedReplicas.length > 0) {
      console.log(
        chalk.red(
          `\n❌ Failed to process ${failedReplicas.length} replica${
            failedReplicas.length > 1 ? "s" : ""
          }:`
        )
      );
    ...
    if (allUploadResults.length > 0) {
      console.log(
        chalk.blue(
          `\n🔄 Monitoring training progress for all ${
            allUploadResults.length
          } replica${allUploadResults.length > 1 ? "s" : ""}...`
        )
      );
      ...
      for (const replicaData of allUploadResults) {
        ...
        try {
          ...
          console.log(
            chalk.yellow(
              `⚠️  Training status monitoring for ${replicaData.replicaName} ended with error: ${error.message}`
            )
          );
        }
      }
...
export function setupSimpleOrganizationSetupCommand(program: Command): void {
  const cmd = program
    ...
    .description(
      "Set up user and multiple replicas with training data from subfolders"
    )
    .option("-u, --user-name <name>", "user name for the account")
    .option("-e, --user-email <email>", "user email address")
    .option(
      "-f, --force",
      "skip confirmation before deleting existing training data"
    )
